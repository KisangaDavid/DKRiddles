...
std::vector<uint32_t> getRandomPlay(std::vector<uint32_t> piles, uint32_t randSource) {
    ...
    for (std::uint32_t i = 0; i < NUM_PILES; ++i) {
        if (piles[i] > 0) {
...
extern "C" {
     __attribute__((visibility("default"))) uint32_t getInitialPiles(uint32_t randSource) {
...
extern "C" {
     __attribute__((visibility("default"))) uint32_t getRoosterRiddleMove(uint32_t inputPiles, uint32_t randSource) {
